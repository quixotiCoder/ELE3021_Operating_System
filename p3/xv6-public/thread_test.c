#include "types.h" // 사용할 데이터 type 정의
#include "stat.h" // user.h에서 사용하는 stat 변수 등 파일 시스템 관리
#include "user.h" // 정의된 user가 실행가능한 시스템콜
...
void failed() // 실패 시 호출 함수
{
  printf(1, "Test failed!\n");
  exit();
}

void *thread_basic(void *arg) // for TEST 1
{
  int val = (int)arg;
  printf(1, "Thread %d start\n", val);
  if (val == 1) { // 쓰레드 1
    sleep(200); // for 2초 후 종료
    status = 1;
  }
...
void *thread_fork(void *arg)
{
  ...
  if (pid == 0) { // thraed가 만든 자식 프로세스 정상 할당 시
...
void create_all(int n, void *(*entry)(void *))
{
  int i;
  for (i = 0; i < n; i++) {
    if (thread_create(&thread[i], entry, (void *)i) != 0) { // 순차적으로 i마다 create_thread
